...
The architecture follows a **Cloud-First, Monorepo design pattern** using pure JavaScript ES Modules (`.js`) and Python microservices:

```
                               ┌────────────────────────────────┐
                               │   React 18 Web Frontend (Vite) │
                               │   Port 3000 | Pure JS / CSS    │
                               └───────────────┬────────────────┘
                                               │ HTTP / WebSockets
                                               ▼
                               ┌────────────────────────────────┐
...
- **Web Frontend**: React 18, Vite, Vanilla CSS + Tailwind, Lucide Icons, Framer Motion (Dark Mode with `Outfit`/`Inter` typography).
- **API Gateway & BFF**: Fastify v4, Fastify CORS, Fastify WebSockets, Zod DTO Validation, LRU + Supabase PostgreSQL Idempotency Middleware.
...
- **A04: Cryptographic Failures**: Flags hardcoded API keys (`sk_live_`) and insecure PRNG (`Math.random()`).
...
│   │   ├── src/server.js     # Gateway routes (/initialize, /lock, /escrow, /settle)
...
│   ├── ci-worker/            # Zero-Trust CI Sandbox Worker
│   │   ├── src/worker.js     # 4-stage pipeline event listener
│   │   ├── src/ast-analyzer.ts # AST complexity & maintainability calculator
│   │   ├── src/security-auditor.ts # OWASP 2025 security scanner
│   │   └── src/sandbox-runner.js   # Ephemeral Docker container manager
│   ├── settlement-worker/    # Oracle Escrow Settlement Engine
│   └── web/                  # React 18 / Vite Web Frontend (Port 3000)
│       ├── src/App.jsx       # Main 4-phase dashboard component
│       └── src/components/   # ContractForm, VerificationDashboard, XaiTrustScoreView, EscrowVaultView
├── packages/
...
npm run verify
```
*Executes Frontend build check, 0 TypeScript file compliance, 100-freelancer matchmaker benchmark, QR-NGC protocol test, and 100-contract system load benchmark.*
