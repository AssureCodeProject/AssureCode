...
> ### ⚠️ This is a proposals document, not a results document
>
> Every figure below under **"Impact"** or **"Targeted Improvement"** is a
> *projection* — an estimate of what an optimization would buy if it were
> implemented. **None of them were measured on this system.** The `22 MB` ONNX
> footprint, the `< 5 ms` relay latency, the `3.5×` inference speedup and the
> `98%` Redis round-trip reduction are all of this kind. Do not cite any of them
> as results.
>
> Measured numbers live in `docs/benchmarks/` and §11 of the technical
> specification, each regenerated by a named command.
>
> **Two of the recommendations have since been implemented**, and their sections
> are annotated inline: §2.1 (HNSW index) and §2.2 (Merkle inclusion proofs).
> The other three — `LISTEN/NOTIFY`, ONNX quantization, batched `XACK` — remain
> unimplemented.
...
> **Status: IMPLEMENTED.** `idx_rag_embeddings_hnsw` exists and the scope guard's
> retrieval path genuinely uses it. The analysis above is therefore historical.
> The "sub-5ms" figure remains a projection — no before/after latency comparison
> against a sequential scan was ever run on this system, and any document
> claiming a measured speedup factor for it is wrong.
...
> **Status: IMPLEMENTED, with a different construction.** Phase 8 added a Merkle
> tree with `getInclusionProof()` / `verifyInclusionProof()`, and the root is
> signed with ML-DSA-87. It is an **RFC 6962** (Certificate Transparency) binary
> tree with domain-separated leaf and node hashing — **not** the Sparse Merkle
> Tree or Patricia trie proposed here. RFC 6962 was chosen because promotion of
> odd nodes rather than duplication avoids CVE-2012-2459. The linear hash chain
> is retained underneath, as recommended.
...
- **Impact**:
  ...
  - Aligns with the **8 GB RAM lightweight constraint**.

> **Status: NOT IMPLEMENTED.** There is no ONNX runtime, no `optimum` dependency,
> and no quantized model anywhere in this repository. The `~22 MB` footprint and
> the `3.5×` latency figures are projections from the ONNX Runtime literature,
> not measurements of this system. (The earlier cross-reference here pointed at
> `LIGHTWEIGHT-TEAM-CONFIG.md`, which does not exist.)
...
| # | Tier | Item | Status |
|---|---|---|---|
| 1a | Database | `HNSW` vector index on `rag_embeddings` | **Done** — `idx_rag_embeddings_hnsw`, queried by the scope guard |
| 1b | Database | `LISTEN/NOTIFY` trigger on `outbox` | **Not done** — `OutboxRelay` still polls at 500 ms |
| 2 | AI/ML | ONNX INT8 quantization of Sentence-BERT | **Not done** — no ONNX runtime in the repository |
| 3 | Ledger | $O(\log N)$ inclusion proofs alongside the linear chain | **Done, different construction** — RFC 6962 binary tree, not an SMT |
| 4 | Event Bus | Batched Redis Streams `XACK` | **Not done** — `xack` is still called per message |

Items 1b, 2 and 4 remain open. Their projected impact figures above have not
been validated and should not be quoted as results.
